...
void WDL_fft_complexmul(WDL_FFT_COMPLEX *a,WDL_FFT_COMPLEX *b,int n)
{
  ...
  while ((n -= 2)>1)
  {
    WDL_FFT_REAL tmp, tmp2;
    tmp = a[0].re * b[0].re - a[0].im * b[0].im;
    tmp2 = a[1].re * b[1].re - a[1].im * b[1].im;
    a[0].im = a[0].im * b[0].re + a[0].re * b[0].im;
    a[1].im = a[1].im * b[1].re + a[1].re * b[1].im;
    a[0].re = tmp;
    a[1].re = tmp2;
  }
}
...
void WDL_fft_complexmul2(WDL_FFT_COMPLEX *c, WDL_FFT_COMPLEX *a, WDL_FFT_COMPLEX *b, int n)
{
  ...
  while ((n -= 2)>0)
  {
    ...
    c[0].im = a[0].im * b[0].re + a[0].re * b[0].im;
    c[1].re = a[1].re * b[1].re - a[1].im * b[1].im;
    c[1].im = a[1].im * b[1].re + a[1].re * b[1].im;
