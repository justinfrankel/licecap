...
#define MEMCTX_RECENT 128 

#ifdef MEMCTX_RECENT
  #if MEMCTX_RECENT < 1 || MEMCTX_RECENT > 255
  #error MEMCTX_RECENT must be > 0 and <= 255
  #endif
  #define MEMCTX_RECENT_MINLEN 2 // no point in shortening 'lines' less than 2 chars long (i.e. >)
#endif
...
#ifdef MEMCTX_RECENT
  WDL_String m_recents[MEMCTX_RECENT];
  int m_recents_pos;
#endif
};
...
#ifdef MEMCTX_RECENT
  unsigned char tmp[2];
  if (l > MEMCTX_RECENT_MINLEN)
  {
    int a = m_recents_pos,x;
    for (x=0;x<MEMCTX_RECENT;x++)
    {
      char *p = m_recents[a].Get();
      if (!strcmp(p,buf))
      {
        l=2;
        tmp[0]=0;
      ...
      if (--a<0) a=MEMCTX_RECENT-1;
    }
    m_recents[m_recents_pos].Set(buf);
    if (++m_recents_pos >= MEMCTX_RECENT) m_recents_pos=0;
  }
#endif
...
#ifdef MEMCTX_RECENT
  if (!cpl && m_pos < m_heapbuf->GetSize()-1)
  {
    // use history
    int a = ((unsigned char *)curptr)[1];
    if (a > 0 && a<=MEMCTX_RECENT)
    {
      int p = m_recents_pos - (a-1);
      if (p<0) p+=MEMCTX_RECENT;
      lstrcpyn(buf,m_recents[p].Get(),buflen);
      m_recents[m_recents_pos].Set(m_recents[p].Get());
...
#ifdef MEMCTX_RECENT
  if (cpl >= MEMCTX_RECENT_MINLEN)
  {
    m_recents[m_recents_pos].Set(curptr);  
    if (++m_recents_pos >= MEMCTX_RECENT) m_recents_pos=0;
  }
#endif
  m_pos += cpl+1;
