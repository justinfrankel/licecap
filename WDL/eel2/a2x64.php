	...
	fputs($out,$line);
      ...
      if ($sline && $lastchunk && strlen($sline) != strlen($lastchunk))
      {
		...
		if ($suffixstr != "dword " || strstr($plist[0],"[")) 
                  makeregs64($plist[0]);
		if ($suffixstr != "dword " || stristr($plist[0],"ffffffff")) makeregs64($plist[1]);
...
process_file("asm-nseel-x86-gcc.c" , "asm-nseel-x64.asm");
system("nasm -f win64 asm-nseel-x64.asm");
//process_file("asm-miscfunc-x86-gcc.c" , "asm-miscfunc-x64.asm");
//process_file("asm-megabuf-x86-gcc.c" , "asm-megabuf-x64.asm");
